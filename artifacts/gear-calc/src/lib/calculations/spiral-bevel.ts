...
export function calculateSpiralBevel(input: SpiralBevelInput): CalculationResult[] {
  ...
  return [
    ...
    // ── Equivalent Machining Lead ────────────────────────────────────────────
    // L = π × D / tan(β)  where D = mn × z (pitch diameter from module × teeth)
    // This is a SETUP value for machining calculations only.
    // A spiral bevel gear does not have a single constant physical lead.
    {
      label: "Pitch Diameter — Pinion (Lead Ref)",
      symbol: "D1",
      formula: "mn × z1",
      variables: "mn = Normal module, z1 = Pinion teeth",
      substitution: `${fmt(mn)} × ${z1}`,
      value: mn * z1,
      unit: "mm",
      note: "Used for equivalent machining lead calculation (always in mm)",
    },
    {
      label: "Pitch Diameter — Gear (Lead Ref)",
      symbol: "D2",
      formula: "mn × z2",
      variables: "mn = Normal module, z2 = Gear teeth",
      substitution: `${fmt(mn)} × ${z2}`,
      value: mn * z2,
      unit: "mm",
      note: "Used for equivalent machining lead calculation (always in mm)",
    },
    {
      label: "Equiv. Machining Lead — Pinion (mm)",
      symbol: "L1",
      formula: "π × D1 / tan(β)",
      variables: "D1 = mn × z1 (pitch diameter), β = Spiral angle",
      substitution: `π × ${fmt(mn * z1)} / tan(${spiralAngle}°)`,
      value: (Math.PI * mn * z1) / Math.tan(beta),
      unit: "mm",
      green: true,
      note: "Equivalent machining lead — for setup calculations only. Not the physical constant lead of the gear.",
    },
    {
      label: "Equiv. Machining Lead — Pinion (in)",
      symbol: "L1",
      formula: "(π × D1 / tan(β)) ÷ 25.4",
      variables: "D1 = Pinion pitch diam (mm), β = Spiral angle",
      substitution: `${fmt((Math.PI * mn * z1) / Math.tan(beta))} ÷ 25.4`,
      value: (Math.PI * mn * z1) / Math.tan(beta) / 25.4,
      unit: "in",
      green: true,
      note: "Inch conversion of pinion equivalent machining lead",
    },
    {
      label: "Equiv. Machining Lead — Gear (mm)",
      symbol: "L2",
      formula: "π × D2 / tan(β)",
      variables: "D2 = mn × z2 (pitch diameter), β = Spiral angle",
      substitution: `π × ${fmt(mn * z2)} / tan(${spiralAngle}°)`,
      value: (Math.PI * mn * z2) / Math.tan(beta),
      unit: "mm",
      green: true,
      note: "Equivalent machining lead — for setup calculations only. Not the physical constant lead of the gear.",
    },
    {
      label: "Equiv. Machining Lead — Gear (in)",
      symbol: "L2",
      formula: "(π × D2 / tan(β)) ÷ 25.4",
      variables: "D2 = Gear pitch diam (mm), β = Spiral angle",
      substitution: `${fmt((Math.PI * mn * z2) / Math.tan(beta))} ÷ 25.4`,
      value: (Math.PI * mn * z2) / Math.tan(beta) / 25.4,
      unit: "in",
      green: true,
      note: "Inch conversion of gear equivalent machining lead",
    },
